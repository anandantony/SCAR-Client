...
namespace NatSuite.ML.Vision {
    ...
    public sealed class NanoDetPredictor : IMLPredictor<(Rect rect, string label, float score)[]> {
        ...
        public unsafe (Rect rect, string label, float score)[] Predict (params MLFeature[] inputs) {
            ...
            foreach (var (stride, logits, displacements, anchors) in new [] {
                ...
                for (var i = 0; i < anchors.Length; ++i) {
                    ...
                    var label = Enumerable
                        .Range(0, logits.shape[2])
                        .Aggregate((p, q) => logits[0,i,p] > logits[0,i,q] ? p : q);
                    ...
                    var rawBox = Rect.MinMaxRect(
                        x1 / inputType.width,
                        1f - y2 / inputType.height,
                        x2 / inputType.width,
                        1f - y1 / inputType.height
                    );
                    var box = rawBox.RectifyAspect(inputType, imageType.width, imageType.height, MLImageFeature.AspectMode.AspectFit);
                    // Add
            ...
            var keepIdx = MLPredictorExtensions.NonMaxSuppression(candidateBoxes, candidateScores, maxIoU);
